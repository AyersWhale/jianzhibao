...
export default class Qiyeshenhe extends Component {
    constructor(props) {
        super(props);
    ...
    getContractList(type) {
        Fetch.getJson(Config.mainUrl + '/companyInfoBF/getCompanyINfoList?type=' + type)
            .then((res) => {
                console.log(res)
    ...
    */
            ...
            <View style={{ flex: 1 }}>
                {/* <ImageBackground source={require('../image/TopBg.png')} style={{ width: deviceWidth, height: 70 + SafeArea.top }}>
                    ...
                    <View style={{ marginTop: 40, alignSelf: 'center', alignContent: 'center', alignItems: 'center' }}>
                        <Text style={{ color: 'white', backgroundColor: 'transparent', fontSize: Config.MainFontSize + 1, fontWeight: 'bold' }}>企业审核</Text>
                    </View>
                </ImageBackground> */}
                <View style={{ width: deviceWidth, height: Platform.OS == 'ios' ? (40 + SafeArea.top) : 70, backgroundColor: '#3E7EFE', alignSelf: 'center' }}>
                    ...
                    <View style={{ marginTop: Platform.OS == 'ios' ? SafeArea.top : 35, alignSelf: 'center', alignContent: 'center', alignItems: 'center' }}>
                        <Text style={{ color: 'white', backgroundColor: 'transparent', fontSize: Config.MainFontSize + 7, fontWeight: 'bold' }}>企业审核</Text>
                    </View>
                </View>
                <View style={{ width: deviceWidth, backgroundColor: 'white', marginBottom: 120 + SafeArea.top }}>
                    <SegmentedControl
                        ref={'C2SegmentedControl'}
                        itemDatas={[{ name: '未审核' }, { name: '企业列表' }]}
    ...
    _SelectPlanItem(selectNum) {
        ...
        if (selectNum == "0") { this.getContractList(0) }
        if (selectNum == "1") { this.getContractList(1) }
    }
    showList() {
    ...
    _renderItem(rowData) {
        console.warn(rowData.isFddStatus)
        return (
            <View>
                <TouchableOpacity style={{ backgroundColor: 'white', borderRadius: 20, width: deviceWidth - 10, marginLeft: 5 }} onPress={() => Actions.UndeterminedContract0({ rowData: rowData, onblock: this.getContractList.bind(this, 0), types: this.state.selectNum, selectNum: this.state.selectNum })}>
                    <View style={{ backgroundColor: '#007ACC', width: deviceWidth / 4, justifyContent: 'center', alignItems: 'center', borderRadius: 5 }}>
                        <Text style={{ fontSize: 12, color: '#fff', }}>{rowData.isFddStatus == '2' ? '已完成实名认证' : "未完成实名认证"}</Text>
                    </View>
                    <View style={{ backgroundColor: 'transparent', marginLeft: 10, paddingTop: 10 }}>
