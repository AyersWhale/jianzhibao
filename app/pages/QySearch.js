...
import { QuickSearch, VectorIcon, Actions } from 'c2-mobile';
...
export default class QySearch extends Component {
    ...
    searchView() {
        if (this.props.theme == 1) {
            ...
            return (
                <TouchableOpacity style={styles.container1} onPress={() => { Actions.JobSearch() }}>
                    <View style={{
                        backgroundColor: 'white', flexDirection: 'row', height: 26, borderRadius: 20
                    }}  >
                        <VectorIcon name='ios-search' size={20} style={{ alignSelf: 'center', marginLeft: 15, color: '#999' }} />
                        <Text
                            ref={"testref"}
                            ...
                            returnKeyLabel={'search'}
                            onChangeText={this.getValue.bind(this)}
                        >{this.props.placeholderText}</Text>
                    </View>
                    <View>
                        {this.loading()}
                    </View>
    ...
    getValue(value) {
        ...
        var datas = QuickSearch.search(this.props.rowData, [['POSITION_NAME', 'like', value], '||', ['COMPANY_NAME', 'like', value], '||', ['POSITION_CITY_NAME', 'like', value], '||', ['POSITION_PROVINCE_NAME', 'like', value], ['POSITION_PROVINCE_AREA', 'like', value]]);
    ...
    getValueResume(value) {
        ...
        var datas = QuickSearch.search(this.props.rowData, [['intentPost', 'like', value], '||', ['personName', 'like', value], '||', ['sex', 'like', value],]);
        if (datas == 0) {
            this.setState({
                loading: false,
                loaded: false
