...
var cityStr =
    [{ "name": "北京市", "city": [{ "name": "北京", "area": ["东城区", "西城区", "崇文区", "宣武区", "朝阳区", "丰台区", "石景山区", "海淀区", "门头沟区", "房山区", "通州区", "顺义区", "昌平区", "大兴区", "平谷区", "怀柔区", "密云县", "延庆县"] }] },
    { "name": "天津市", "city": [{ "name": "天津", "area": ["和平区", "河东区", "河西区", "南开区", "河北区", "红桥区", "塘沽区", "汉沽区", "大港区", "东丽区", "西青区", "津南区", "北辰区", "武清区", "宝坻区", "宁河县", "静海县", "蓟  县"] }] },
    {
        "name": "河北省", "city": [{ "name": "石家庄", "area": ["长安区", "桥东区", "桥西区", "新华区", "郊  区", "井陉矿区", "井陉县", "正定县", "栾城县", "行唐县", "灵寿县", "高邑县", "深泽县", "赞皇县", "无极县", "平山县", "元氏县", "赵  县", "辛集市", "藁", "晋州市", "新乐市", "鹿泉市"] },
...
class CityInfo extends Component {
        ...
        for (i = 0; i < cityStr.length; i++) {
            if (province === cityStr[i].name) {
                ...
                for (j = 0; j < cities.length; j++) {
                    cityNames.push(cities[j].name + '市');
                }
                break;
            }
        }
    ...
    static getCityWithid(id) {
        //市
        var provinceNames = [];
        fetch(Config.mainUrl + '/ws/city?parentid=' + id, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            }
    ...
    static getAreaWithid(id) {
        //县
        var provinceNames = [];
        fetch(Config.mainUrl + '/ws/area?parentid=' + id, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            }
